...
export function BahanBakuInnerForm({
  existingKategori = [],
  existingSatuan = [],
  initialData = null,
  onSuccess = () => { },
  ...
  const [satuanDasar, setSatuanDasar] = useState((initialData?.satuanDasar === "gram" ? "" : initialData?.satuanDasar) || "")
  ...
  async function actionSubmit(formData: FormData) {
    ...
    try {
      if (initialData) {
        await db.bahanBaku.update(initialData.id, {
        ...
        toast.add({
          title: "Bahan baku berhasil diubah!",
          type: "success"
        })
      } else {
        await db.bahanBaku.add({
      ...
      console.error(error)
      toast.add({
        title: "Gagal menyimpan bahan baku",
  ...
  return (
    <form action={actionSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center min-h-[20px]">
            <Label htmlFor="kategori">Kategori Bahan</Label>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center min-h-[20px]">
            <Label htmlFor="satuanDasar">Nama Satuan</Label>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center min-h-[20px]">
            <Label htmlFor="konversi">Jumlah Satuan</Label>
          </div>
          <Input
            type="text"
            inputMode="numeric"
      ...
      {!isOpen && (
        <div className="flex flex-col gap-4 mb-4">
          <div className="flex justify-end items-center">
            <Button onClick={() => setOpen(true)}>Tambah Bahan Baku</Button>
          </div>
        </div>
      )}
      {isOpen && (
        <div className="border p-4 md:p-6 bg-card text-card-foreground">
          <div className="mb-4">
            <h2 className="text-lg font-semibold">{itemToEdit ? "Edit Bahan Baku" : "Tambah Bahan Baku"}</h2>
          </div>
