  ...
  const executeDelete = () => {
    if (itemToDelete) {
      startTransition(async () => {
        try {
          await db.bahanBaku.delete(itemToDelete.id)
          ...
          toast.add({
            title: "Gagal menghapus bahan baku",
            type: "error"
          })
        }
      })
      ...
      <div className="hidden md:block border mt-4 rounded-none overflow-hidden bg-card shadow-sm">
        <Table className="[&_th]:border-r [&_td]:border-r [&_th:last-child]:border-r-0 [&_td:last-child]:border-r-0">
          <TableHeader className="bg-muted text-foreground">
            <TableRow>
              <TableHead className="text-left w-[250px]">Nama Bahan</TableHead>
              <TableHead className="text-center w-[140px]">Kategori Bahan</TableHead>
      ...
      <AlertDialog open={!!itemToDelete} onOpenChange={(open) => !open && setItemToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tindakan ini tidak dapat dibatalkan. Data bahan baku ini akan dihapus secara permanen dari server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
